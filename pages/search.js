...
import {
  ...
  UserCircleIcon,
  SearchIcon,
  DocumentSearchIcon,PhotographIcon,VideoCameraIcon,NewspaperIcon,MapIcon,CogIcon,ArrowLeftIcon,ArrowRightIcon
    ...
    <div className="app" ref={appDiv}>
      ...
      <header ref={headerRef} className={styles.searchHeader}>
        ...
        <span className={styles.headerSearch}>
          ...
          <img onClick={()=> router.push('/')} className={styles.searchLogo} src="/google.png"/>
          <form onSubmit={search} className={`search ${styles.search} ${styles.desktop}`}>
            <input type="search" value={userInp} onChange={(e)=> setUserInp(e.target.value)}/>
          </form>
        </span>
        <span>
...
<li className={`icon ${styles.icon} ${styles.active}`}><DocumentSearchIcon/> <span>All</span></li>
<li className={`icon ${styles.icon}`}><PhotographIcon/> <span>Images</span></li>
<li className={`icon ${styles.icon}`}><VideoCameraIcon/> <span>Videos</span></li>
<li className={`icon ${styles.icon}`}><NewspaperIcon/> <span>News</span></li>
<li className={`icon ${styles.icon}`}><MapIcon/> <span>Maps</span></li>
<li className={`icon ${styles.icon}`}><CogIcon/> <span>Search Options</span></li>
</ul>

<span className={styles.time}>About {results?.searchInformation?.formattedTotalResults} results in {results?.searchInformation?.searchTime} seconds</span>

<ul className={styles.results} ref={resultsDiv}>
{results?.items?.map((result) => (
<li key={result.link}><a href={result.link}>
...
{queryStartIndex >= 10 && <button className={`icon ${styles.icon}`} onClick={goPrev}><ArrowLeftIcon/></button>}
<button className={`icon ${styles.icon}`} onClick={goNext}><ArrowRightIcon/></button>
</div>
...
export const getServerSideProps = async (context) => {
  ...
  const data = await fetch(`https://www.googleapis.com/customsearch/v1?key=${process.env.CSE_API_KEY}&cx=${process.env.CSE_CONTEXT_KEY}&q=${context.query.q}&start=${startIndex}`).then( response => response.json());
  return {
    props: {results:data}
  }
}